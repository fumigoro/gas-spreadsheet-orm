...
export class QueryEngine<T> {
	...
	private evaluateCondition<V>(
		...
		if (typeof value === "number" || value instanceof Date) {
			if (
				"lt" in condition &&
				(typeof condition.lt === "number" || condition.lt instanceof Date)
			) {
				return value < condition.lt;
			}
			if (
				"lte" in condition &&
				(typeof condition.lte === "number" || condition.lte instanceof Date)
			) {
				return value <= condition.lte;
			}
			if (
				"gt" in condition &&
				(typeof condition.gt === "number" || condition.gt instanceof Date)
			) {
				return value > condition.gt;
			}
			if (
				"gte" in condition &&
				(typeof condition.gte === "number" || condition.gte instanceof Date)
			) {
		...
		if (typeof value === "string") {
			...
			if (
				...
				typeof condition.startsWith === "string"
			) {
	...
	sort(orderBy?: { [K in keyof T]?: "asc" | "desc" }): T[] {
		...
		return [...this.data].sort((a, b) => {
			for (const [key, direction] of Object.entries(orderBy) as Array<
				[keyof T, "asc" | "desc"]
			>) {
				const aVal = a[key];
				const bVal = b[key];
