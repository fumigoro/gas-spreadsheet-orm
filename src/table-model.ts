...
export class TableModel<T extends Record<string, ColumnDefinition>> {
  ...
  private mapRowToObject(row: unknown[]): InferModelType<T> {
    ...
    for (const [fieldName, columnDef] of Object.entries(this.schema) as Array<[keyof T, ColumnDefinition]>) {
  ...
  private findRowIndex(id: InferModelType<T>[keyof T]): number {
    return this.data.findIndex(item => item[this.primaryKey as keyof InferModelType<T>] === id);
  }

  // Public API methods (Prisma-like)
  async findMany(args?: FindManyArgs<InferModelType<T>>): Promise<InferModelType<T>[]> {
  ...
  async findUnique(args: FindUniqueArgs<InferModelType<T>>): Promise<InferModelType<T> | null> {
    const queryEngine = new QueryEngine(this.data);
    const result = queryEngine.findFirst(args.where);
    return result || null;
  }

  async findFirst(args: FindUniqueArgs<InferModelType<T>>): Promise<InferModelType<T> | null> {
    return this.findUnique(args);
  }

  async create(args: CreateArgs<InferModelType<T>>): Promise<InferModelType<T>> {
    const newData = { ...args.data } as Record<keyof T, unknown>;
    
    // Apply defaults
    for (const [fieldName, columnDef] of Object.entries(this.schema) as Array<[keyof T, ColumnDefinition]>) {
      if (columnDef.default !== undefined && newData[fieldName] === undefined) {
        newData[fieldName] = typeof columnDef.default === 'function' 
          ? columnDef.default() 
          : columnDef.default;
      }
    }
    ...
    if (this.primaryKey && newData[primaryKeyField] !== undefined) {
      ...
      const existing = this.data.find(item => item[primaryKeyField] === primaryKeyValue);
      if (existing) {
        throw new Error(`Record with ${String(this.primaryKey)} '${String(primaryKeyValue)}' already exists`);
      }
    }
  ...
  async update(args: UpdateArgs<InferModelType<T>>): Promise<InferModelType<T>> {
    const queryEngine = new QueryEngine(this.data);
  ...
  async delete(args: DeleteArgs<InferModelType<T>>): Promise<InferModelType<T>> {
    const queryEngine = new QueryEngine(this.data);
    const existing = queryEngine.findFirst(args.where);
  ...
  async deleteMany(args?: DeleteManyArgs<InferModelType<T>>): Promise<{ count: number }> {
